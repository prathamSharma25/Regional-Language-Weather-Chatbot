...
def get_response(message):
    user_query_EN = translate_to_EN(message)
    if user_query_EN=='goodbye' or user_query_EN=='see you soon':
        response = translator.translate("Goodbye! Have a nice day!", dest=target_lang).text
    else:
        user_query_EN_tokens = remove_stopwords(tokenize(user_query_EN))
        chatbot_query = most_similar_query(user_query_EN_tokens)
        response = translate_to_target_language(weather_info(chatbot_query))
    return response